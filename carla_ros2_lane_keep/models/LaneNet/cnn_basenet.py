...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.variable_scope(name):
            ...
            # Convert filter shape to int
            filter_shape = [int(dim) if isinstance(dim, float) else dim for dim in filter_shape]
    ...
    @staticmethod
    # @tf.function  # Ensures compatibility with TensorFlow's graph execution
    def layerbn(inputdata, is_training, name, scale=True):
        ...
        # Ensure `training` is passed correctly in graph mode
        output = tf.cond(
            is_training,
            lambda: bn_layer(inputdata, training=True),  # Training mode
            lambda: bn_layer(inputdata, training=False)  # Inference mode
        )

        return output
        # return bn_layer(inputdata, training=is_training)
